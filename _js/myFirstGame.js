...
var update = function (modifier) {
	...
	if (allMonsters.length == 0) {
		...
		wave += 10;
		Monster.gravity += 25;
		console.log("monstergravity is " + Monster.gravity);
		monsterWave(wave);
	}
...
// ################# Render/Draw section ######################
// drawwing the stuff, when stuff loads
var render = function (modifier) {
	...
	if (monsterReady) {
		for (monster in allMonsters) {
			...
			if (bossReady && allMonsters[monster].type == "boss") {
	...
	ctx.textAlign = "left";
	ctx.textBaseline = "top";
	ctx.fillText("Timer: " + currentTimer, 256, 32);
